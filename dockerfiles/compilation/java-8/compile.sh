...
if [ $? -eq 0 ]; then
   if [ -d "$CASSANDRA_DESTINATION" ]; then
      ...
      cp server/container/guice/cassandra-guice/target/james-server-cassandra-ldap-guice.jar $CASSANDRA_DESTINATION || true
      cp -r server/container/guice/cassandra-guice/target/james-server-cassandra-ldap-guice.lib $CASSANDRA_DESTINATION || true
   fi
