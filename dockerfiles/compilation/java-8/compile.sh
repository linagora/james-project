...
if [ "$SKIPTESTS" = "skipTests" ]; then
   mvn package -DskipTests -Pcassandra,elasticsearch,inmemory,exclude-lucene,with-assembly,with-jetm -T1C
else
   mvn package -Pcassandra,inmemory,elasticsearch,exclude-lucene,with-assembly,with-jetm -T1C
fi
