...
public class SetACLProcessorTest {
        ...
        return new Expectations() {
            {
                ...
                will(returnValue(rightManagerStub));
            }
        };
    }
    ...
    private void testOp(String prefix, EditMode editMode) throws MailboxException {
        ...
        expectations.allowing(rightManagerStub).applyRightsCommand(expectations.with(path), expectations.with(Expectations.equal(MailboxACL.command().key(user1Key).mode(editMode).rights(setRights).build())), expectations.with(mailboxSessionStub));
