...
public class GetAnnotationProcessor extends AbstractMailboxProcessor<GetAnnotationRequest> implements CapabilityImplementingProcessor {
    ...
    private void proceed(GetAnnotationRequest message, ImapSession session, String tag, ImapCommand command, Responder responder) throws MailboxException {
        ...
        long maxsize = message.getMaxsize();
        ...
        Optional<Integer> maximumOversizedSize = getMaximumOversizeSize(mailboxAnnotations, maxsize);
    ...
    private void respond(String tag, ImapCommand command, Responder responder, String mailboxName,
                         List<MailboxAnnotation> mailboxAnnotations, long maxsize, Optional<Integer> maximumOversizedSize) {
    ...
    private Optional<Integer> getMaximumOversizeSize(List<MailboxAnnotation> mailboxAnnotations, long maxsize) {
        if (maxsize > 0) {
            return getValuesOverSize(mailboxAnnotations, maxsize);
        } else {
            return Optional.absent();
        }
    }

    private List<MailboxAnnotation> filterItemsBySize(Responder responder, String mailboxName, List<MailboxAnnotation> mailboxAnnotations, final long maxsize) {
    ...
    private Optional<Integer> getValuesOverSize(List<MailboxAnnotation> mailboxAnnotations, final long maxsize) {
