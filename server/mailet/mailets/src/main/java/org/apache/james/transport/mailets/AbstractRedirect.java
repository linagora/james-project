...
public abstract class AbstractRedirect extends GenericMailet {
    ...
    private String getText(Mail originalMail, MimeMessage originalMessage, String head) throws MessagingException {
        ...
        switch (getInLineType(originalMail)) {
            ...
            case BODY:
                appendBody(buffer, originalMessage);
                break;
            case NONE:
                break;
            case MESSAGE:
    ...
    private void appendBody(StringBuffer buffer, MimeMessage originalMessage) {
        buffer.append("Message:")
            .append(LINE_BREAK);
        try {
            buffer.append(getMessageBody(originalMessage))
                .append(LINE_BREAK);
        } catch (Exception e) {
            buffer.append("body unavailable")
                .append(LINE_BREAK);
        }
    }
