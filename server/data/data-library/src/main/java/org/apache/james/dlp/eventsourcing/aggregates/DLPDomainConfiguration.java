...
public class DLPDomainConfiguration {
    ...
    public List<Event> clear() {
        ImmutableList<DLPConfigurationItem> rules = retrieveRules().collect(ImmutableList.toImmutableList());
        if (!rules.isEmpty()) {
    ...
    public List<Event> store(List<DLPConfigurationItem> updatedRules) {
        ImmutableSet<DLPConfigurationItem> existingRules = retrieveRules().collect(ImmutableSet.toImmutableSet());
