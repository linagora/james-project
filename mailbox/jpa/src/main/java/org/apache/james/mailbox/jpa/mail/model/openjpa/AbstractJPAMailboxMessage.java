...
public abstract class AbstractJPAMailboxMessage implements MailboxMessage {
    ...
    public void setFlags(Flags flags) {
        ...
        /*
         * // Loop over the user flags and check which of them needs to get
         * added / removed List<String> uFlags =
         * Arrays.asList(flags.getUserFlags()); for (int i = 0; i <
         * userFlags.size(); i++) { JPAUserFlag f = userFlags.get(i); if
         * (uFlags.contains(f.getName()) == false) { userFlags.remove(f); i++; }
         * } for (int i = 0; i < uFlags.size(); i++) { boolean found = false;
         * String uFlag = uFlags.get(i); for (int a = 0; a < userFlags.size();
         * a++) { String userFlag = userFlags.get(a).getName(); if
         * (userFlag.equals(uFlag)) { found = true; break; } } if (found ==
         * false) { userFlags.add(new JPAUserFlag(uFlag)); }
         * 
         * 
         * }
         */
