...
public class CassandraMessageDAO {
    ...
    private MailboxMessage message(Row row, FetchType fetchType, Function<List<AttachmentId>, List<Attachment>> attachmentsFunction) {
        try {
            ComposedMessageIdWithFlags messageIdWithFlags = retrieveComposedMessageId(CassandraMessageId.of(row.getUUID(MESSAGE_ID))).join();
            ...
            SimpleMailboxMessage message =
                    new SimpleMailboxMessage(
                            ...
                            getAttachments(row, fetchType, attachmentsFunction));
    ...
    private List<MessageAttachment> getAttachments(Row row, FetchType fetchType, Function<List<AttachmentId>, List<Attachment>> attachmentsFunction) {
        switch (fetchType) {
        case Full:
        case Body:
            List<UDTValue> udtValues = row.getList(ATTACHMENTS, UDTValue.class);
            Map<AttachmentId,Attachment> attachmentsById = attachmentsById(row, udtValues, attachmentsFunction);
    ...
    private Map<AttachmentId,Attachment> attachmentsById(Row row, List<UDTValue> udtValues, Function<List<AttachmentId>, List<Attachment>> attachmentsFunction) {
        Map<AttachmentId, Attachment> map = new HashMap<>();
        attachmentsFunction.apply(attachmentIds(udtValues)).stream()
