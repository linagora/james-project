...
public class InMemoryMailboxManagerAttachmentTest extends AbstractMailboxManagerAttachmentTest {
    ...
    public InMemoryMailboxManagerAttachmentTest() throws Exception {
        ...
        mailboxManager = new InMemoryMailboxManager(mailboxSessionMapperFactory, null, new JVMMailboxPathLocker(), new UnionMailboxACLResolver(), null, new MessageParser());
        mailboxManager.init();
        MessageParser failingMessageParser = mock(MessageParser.class);
        when(failingMessageParser.retrieveAttachments(any(InputStream.class)))
            .thenThrow(new RuntimeException("Message parser set to fail"));
        parseFailingMailboxManager = new InMemoryMailboxManager(mailboxSessionMapperFactory, null, new JVMMailboxPathLocker(), new UnionMailboxACLResolver(), null, failingMessageParser);
